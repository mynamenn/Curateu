...
@section('content')
    <script>
        ...
        function showDeleteConfirmation(e, formId) {
            e.stopPropagation();
    ...
    <div class="w-full bg-cover bg-center mb-4" style="height:28rem; background-image: url({{ $user->cover_picture }});">
        <div class="flex items-center justify-center h-full w-full bg-gray-900 bg-opacity-50">
            <div class="flex-column px-4 rounded-lg overflow-hidden align items-center justify-center text-center">
                ...
                <a class="block tracking-widest text-sm title-font font-bold text-white hover:text-blue-500 focus:text-blue-500 focus:underline hover:underline mb-1 md:text-base"
        ...
        @if ($collections->total() > 0)
            <div class="mb-6 border-2 border-gray-300 border-opacity-50 rounded-md">
                @foreach ($collections as $index => $collection)
                    ...
                    @if (AuthHelper::canEditItself($user->id))
                        <div id={{ 'deleteCollection' . $collection->id }} class="hidden">
                            ...
                            <form action="{{ route('collections.destroy', $collection) }}" method="post"
                                ...
                                @method('DELETE')
                                @csrf
                                <p class="text-lg font-semibold mb-2">Are you sure you want to delete this collection?</p>

                                <div class="flex flex-row">
                                    <button type="submit"
                                        class="bg-indigo-500 text-white px-4 py-1 mr-2 rounded-xl text-center items-center transition duration-500 ease-in-out hover:bg-indigo-600">
                                        Yes
                                    </button>
                                    <button type="button"
                                        onclick="closeCollection('{{ 'deleteCollection' . $collection->id }}')"
                                        class="text-indigo-500 font-bold text-base  border border-indigo-700 rounded-xl px-4 py-1 text-center">
                                        No
                                    </button>
                                </div>
                    ...
                    @if ($collections->hasPages() == false && $index == $collections->count() - 1)
                        {{-- Do nothing --}}
                    @else
                        <hr class="border-t-2 border-gray-300 border-opacity-50" />
                    @endif
