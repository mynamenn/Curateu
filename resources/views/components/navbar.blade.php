<script>
    $(document).ready(function() {
        $('#searchInput').keydown(function(event) {
            if (event.which == 13) {
                location.href = '/search?query=' + document.getElementById("searchInput").value;
            }
        });
    });
</script>
...
<header class="text-gray-600 body-font">
    <div class="container mx-auto flex flex-wrap p-5 flex-col lg:flex-row items-center">
        ...
        <div class="lg:ml-auto ml-0 flex flex-wrap items-center text-base justify-center">
            @if (Auth::check())
                <a class="mr-5 hover:text-gray-900 hover:font-bold"
                    href="/{{ '@' }}{{ Auth::user()->username }}">Profile</a>
            @endif

            <a class="mr-5 hover:text-indigo-500 focus:text-indigo-500 focus:font-bold" href="/categories">Categories</a>
            <a class="mr-5 hover:text-indigo-500 focus:text-indigo-500 focus:font-bold"
            ...
            @if (Auth::check())
                <form action="{{ route('logout') }}" method="post" class="mb-0">
                    @csrf
                    <button type="submit"
                        class="lg:mr-5 mr-0 hover:font-bold hover:text-indigo-500 focus:text-indigo-500">
                        Logout
                    </button>
                </form>
            @else
